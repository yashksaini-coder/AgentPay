...
import { useAgent, type AgentState, type DiscoveryOverride } from "@/lib/useAgent";
...
function AgentHook({ port, discovery, onState }: { port: number; discovery?: DiscoveryOverride; onState: (port: number, state: AgentState) => void }) {
  const state = useAgent(port, discovery);
  const keyRef = useRef("");
...
export default function NetworkPage() {
  ...
  const handleOpenChannel = async () => {
    ...
    // In remote mode, only the live node can send/receive real operations
    if (!sender.live || !receiver.live) {
      pushEvent({ type: "status", from: lbl(sender), message: `Channel open skipped: discovered agents are view-only in remote mode` });
      setInteraction(null);
      return;
    }
  ...
  const handleRoutePayment = async () => {
    ...
    if (!sender.live || !receiver.live) {
      pushEvent({ type: "status", from: lbl(sender), message: `Payment skipped: discovered agents are view-only in remote mode` });
      setInteraction(null);
      return;
    }
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      {manager.agents.map((a) => {
        const disc: DiscoveryOverride | undefined = a.peer_id
          ? { peer_id: a.peer_id, eth_address: a.eth_address || "", capabilities: a.capabilities, addrs: a.addrs, live: a.live ?? true }
          : undefined;
        return <AgentHook key={a.apiPort} port={a.apiPort} discovery={disc} onState={handleAgentState} />;
      })}
...
function TrustPanel({ api, trustScores }: { api: ReturnType<typeof import("@/lib/api").createApi>; trustScores: Record<string, number> }) {
  type TrustTab = "overview" | "protocol" | "finance";
  const [tab, setTab] = useState<TrustTab>("overview");

  // Sub-section within each tab
  const [protoSection, setProtoSection] = useState<"discovery" | "negotiations" | "sla" | "disputes">("discovery");
  const [finSection, setFinSection] = useState<"pricing" | "receipts" | "policies">("pricing");

  return (
    <div className="space-y-2 pt-1 border-t border-border-subtle">
      <h3 className="text-[10px] font-bold uppercase tracking-widest text-text-muted">Trust & Protocol</h3>

      {/* Primary tabs — 3 clear groups */}
      <div className="flex rounded-lg bg-surface-overlay/60 p-0.5">
        {([["overview", "Overview"], ["protocol", "Protocol"], ["finance", "Finance"]] as const).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`flex-1 px-2 py-1.5 text-[10px] font-medium rounded-md transition-colors ${
              tab === key ? "bg-surface-hover text-text-primary" : "text-text-muted hover:text-text-secondary"
            }`}
          >
            {label}
          </button>
      ...
      {/* Overview: Identity panel */}
      {tab === "overview" && <IdentityPanel api={api} />}

      {/* Protocol: Discovery / Negotiations / SLA / Disputes */}
      {tab === "protocol" && (
        <div className="space-y-2">
          <div className="flex gap-1">
            {([["discovery", "Discover"], ["negotiations", "Negotiate"], ["sla", "SLA"], ["disputes", "Disputes"]] as const).map(([key, label]) => (
              <button
                key={key}
                onClick={() => setProtoSection(key)}
                className={`px-2 py-1 text-[9px] font-medium rounded-md transition-colors ${
                  protoSection === key ? "bg-accent/10 text-accent" : "text-text-muted/60 hover:text-text-muted"
                }`}
              >
                {label}
              </button>
          ...
          {protoSection === "discovery" && <DiscoveryPanel api={api} trustScores={trustScores} />}
          {protoSection === "negotiations" && <NegotiationTimeline api={api} />}
          {protoSection === "sla" && <SLAPanel api={api} />}
          {protoSection === "disputes" && <DisputePanel api={api} />}
        </div>
      )}

      {/* Finance: Pricing / Receipts / Policies */}
      {tab === "finance" && (
        <div className="space-y-2">
          <div className="flex gap-1">
            {([["pricing", "Pricing"], ["receipts", "Receipts"], ["policies", "Policy"]] as const).map(([key, label]) => (
              <button
                key={key}
                onClick={() => setFinSection(key)}
                className={`px-2 py-1 text-[9px] font-medium rounded-md transition-colors ${
                  finSection === key ? "bg-accent/10 text-accent" : "text-text-muted/60 hover:text-text-muted"
                }`}
              >
                {label}
              </button>
          ...
          {finSection === "pricing" && <PricingPanel api={api} />}
          {finSection === "receipts" && <ReceiptChain api={api} />}
          {finSection === "policies" && <PolicyEditor api={api} />}
        </div>
      )}
    </div>
  );
